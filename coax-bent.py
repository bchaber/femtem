...
boundaries.array()[output_port] = sbc_boundary
...
File(f"results/subdomains.pvd") << subdomains
File(f"results/boundaries.pvd") << boundaries
File(f"results/epsr.pvd") << epsr
File(f"results/sigma.pvd") << sigma
File(f"results/mur.pvd") << mur

for f in [500e6, 800e6]:
  ## Physical constants
  freq = Constant(f)
  omega = 2 * pi * freq
  ...
  if sbc_boundary in boundaries.array():
    ...
    scattering_boundary = ScatteringBoundaryCondition(T_re, T_im, E_re, E_im, n, k, dS0)
  ...
  f.write(f"results/{freq_v}MHz_E-field.vtu")
  print("...done\n")
